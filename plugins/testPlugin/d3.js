! function() {
  ...
  function d3_window(node) {
    return node && (node.ownerDocument && node.ownerDocument.defaultView ||
      node.document && node || node.defaultView);
  }
  if (d3_document) {
    try {
  ...
  d3.median = function(array, f) {
    ...
    if (arguments.length === 1) {
      ...
      while (++i < n)
        if (d3_numeric(a = d3_number(f.call(array, array[i], i)))) numbers.push(
          a);
    }
  ...
  d3.transpose = function(matrix) {
    ...
    for (var i = -1, m = d3.min(matrix, d3_transposeLength), transpose = new Array(
        m); ++i < m;) {
  ...
  d3_class(d3_Map, {
    ...
    empty: d3_map_empty,
    forEach: function(f) {
      for (var key in this._) f.call(this, d3_map_unescape(key), this._[
        key]);
    }
  });

  function d3_map_escape(key) {
    return (key += "") === d3_map_proto || key[0] === d3_map_zero ? d3_map_zero +
      key : key;
  }
  ...
  d3_dispatch.prototype.on = function(type, listener) {
    ...
    if (type) return arguments.length < 2 ? this[type].on(name) : this[type].on(
      name, listener);
    if (arguments.length === 2) {
      if (listener == null)
        for (type in this) {
  ...
  var d3_select = function(s, n) {
    ...
    d3_selectMatches = function(n, s) {
      var d3_selectMatcher = n.matches || n[d3_vendorSymbol(n,
        "matchesSelector")];
      d3_selectMatches = function(n, s) {
  ...
  d3.ns = {
    ...
    qualify: function(name) {
      var i = name.indexOf(":"),
        prefix = name;
      if (i >= 0 && (prefix = name.slice(0, i)) !== "xmlns") name = name.slice(
        i + 1);
  ...
  d3_selectionPrototype.style = function(name, value, priority) {
    ...
    if (n < 3) {
      if (typeof name !== "string") {
        if (n < 2) value = "";
        for (priority in name) this.each(d3_selection_style(priority, name[
          priority], value));
        return this;
      }
      if (n < 2) {
        var node = this.node();
        return d3_window(node).getComputedStyle(node, null).getPropertyValue(
          name);
      }
      priority = "";
    }
  ...
  function d3_selection_creator(name) {
    function create() {
      ...
      return namespace === d3_nsXhtml && document.documentElement.namespaceURI ===
        d3_nsXhtml ? document.createElement(name) : document.createElementNS(
          namespace, name);
    }
    ...
    return typeof name === "function" ? name : (name = d3.ns.qualify(name)).local ?
      createNS : create;
  }
  ...
  d3_selectionPrototype.order = function() {
    for (var j = -1, m = this.length; ++j < m;) {
      for (var group = this[j], i = group.length - 1, next = group[i], node; --
        i >= 0;) {
        if (node = group[i]) {
  ...
  function d3_event_dragSuppress(node) {
    var name = ".dragsuppress-" + ++d3_event_dragId,
      click = "click" + name,
      w = d3.select(d3_window(node)).on("touchmove" + name,
        d3_eventPreventDefault).on("dragstart" + name, d3_eventPreventDefault).on(
        "selectstart" + name, d3_eventPreventDefault);
    if (d3_event_dragSelect == null) {
      d3_event_dragSelect = "onselectstart" in node ? false : d3_vendorSymbol(
  ...
  d3.behavior.drag = function() {
    var event = d3_eventDispatch(drag, "drag", "dragstart", "dragend"),
      origin = null,
      mousedown = dragstart(d3_noop, d3.mouse, d3_window, "mousemove",
        "mouseup"),
      touchstart = dragstart(d3_behavior_dragTouchId, d3.touch, d3_identity,
        "touchmove", "touchend");
    ...
    function dragstart(id, position, subject, move, end) {
      return function() {
        var that = this,
          ...
          dragOffset, dragSubject = d3.select(subject(target)).on(move +
  ...
  d3.behavior.zoom = function() {
        ...
        if (d3_transitionInheritId) {
          d3.select(this).transition().each("start.zoom", function() {
            ...
            var dx = size[0],
              ...
              i = d3.interpolateZoom([(cx - view.x) / view.k, (cy -
                view.y) / view.k, dx / view.k], [(cx - view1.x) /
                view1.k, (cy - view1.y) / view1.k, dx / view1.k
              ]);
            return function(t) {
              var l = i(t),
                k = dx / l[2];
    ...
    function mousedowned() {
      var that = this,
        dispatch = event.of(that, arguments),
        dragged = 0,
        subject = d3.select(d3_window(that)).on(mousemove, moved).on(mouseup,
          ended),
  ...
  function d3_hsl(h, s, l) {
    return this instanceof d3_hsl ? void(this.h = +h, this.s = +s, this.l = +l) :
      ...
      d3_rgb_parse("" + h, d3_rgb_hsl, d3_hsl) : new d3_hsl(h, s, l);
  }
  ...
  function d3_hcl(h, c, l) {
    return this instanceof d3_hcl ? void(this.h = +h, this.c = +c, this.l = +l) :
      ...
      h instanceof d3_lab ? d3_lab_hcl(h.l, h.a, h.b) : d3_lab_hcl((h =
        d3_rgb_lab((h = d3.rgb(h)).r, h.g, h.b)).l, h.a, h.b) : new d3_hcl(h, c,
        l);
  }
  ...
  function d3_lab(l, a, b) {
    return this instanceof d3_lab ? void(this.l = +l, this.a = +a, this.b = +b) :
      ...
      l instanceof d3_hcl ? d3_hcl_lab(l.h, l.c, l.l) : d3_rgb_lab((l = d3_rgb(
        l)).r, l.g, l.b) : new d3_lab(l, a, b);
  }
  var d3_lab_K = 18;
  var d3_lab_X = .95047,
    d3_lab_Y = 1,
  ...
  d3_labPrototype.brighter = function(k) {
    return new d3_lab(Math.min(100, this.l + d3_lab_K * (arguments.length ? k :
      1)), this.a, this.b);
  };
  d3_labPrototype.darker = function(k) {
    return new d3_lab(Math.max(0, this.l - d3_lab_K * (arguments.length ? k :
      1)), this.a, this.b);
  };
  ...
  function d3_lab_rgb(l, a, b) {
    ...
    return new d3_rgb(d3_xyz_rgb(3.2404542 * x - 1.5371385 * y - .4985314 * z),
      d3_xyz_rgb(-.969266 * x + 1.8760108 * y + .041556 * z), d3_xyz_rgb(.0556434 *
        x - .2040259 * y + 1.0572252 * z));
  }

  function d3_lab_hcl(l, a, b) {
    return l > 0 ? new d3_hcl(Math.atan2(b, a) * d3_degrees, Math.sqrt(a * a +
      b * b), l) : new d3_hcl(NaN, NaN, l);
  }
  ...
  function d3_xyz_rgb(r) {
    return Math.round(255 * (r <= .00304 ? 12.92 * r : 1.055 * Math.pow(r, 1 /
      2.4) - .055));
  }
  d3.rgb = d3_rgb;
  ...
  function d3_rgb(r, g, b) {
    return this instanceof d3_rgb ? void(this.r = ~~r, this.g = ~~g, this.b = ~
      ~b) : arguments.length < 2 ? r instanceof d3_rgb ? new d3_rgb(r.r, r.g,
      r.b) : d3_rgb_parse("" + r, d3_rgb, d3_hsl_rgb) : new d3_rgb(r, g, b);
  }
  ...
  function d3_rgb_parse(format, rgb, hsl) {
    ...
    if (m1) {
      ...
      switch (m1[1]) {
        case "hsl":
          {
            return hsl(parseFloat(m2[0]), parseFloat(m2[1]) / 100, parseFloat(
              m2[2]) / 100);
          }
    ...
    if (format != null && format.charAt(0) === "#" && !isNaN(color = parseInt(
        format.slice(1), 16))) {
      if (format.length === 4) {
        r = (color & 3840) >> 4;
        r = r >> 4 | r;
  ...
  function d3_xhrType(response) {
    return function(url, mimeType, callback) {
      if (arguments.length === 2 && typeof mimeType === "function") callback =
        mimeType,
        mimeType = null;
  ...
  function d3_xhr(url, mimeType, response, callback) {
    var xhr = {},
      ...
      responseType = null;
    if (this.XDomainRequest && !("withCredentials" in request) &&
  ...
  d3.dsv = function(delimiter, mimeType) {
    ...
    function dsv(url, row, callback) {
      ...
      var xhr = d3_xhr(url, mimeType, row == null ? response : typedResponse(
        row), callback);
      xhr.row = function(_) {
  ...
  var d3_timer_queueHead, d3_timer_queueTail, d3_timer_interval,
    d3_timer_timeout, d3_timer_frame = this[d3_vendorSymbol(this,
  ...
  var d3_formatPrefixes = ["y", "z", "a", "f", "p", "n", "µ", "m", "", "k", "M",
    "G", "T", "P", "E", "Z", "Y"
  ].map(d3_formatPrefix);
  ...
  function d3_locale_numberFormat(locale) {
    ...
    return function(specifier) {
      ...
      if (precision != null) {
        ...
        else if (type == "e" || type == "f") precision = Math.max(0, Math.min(
          20, precision));
      }
      ...
      return function(value) {
        ...
        var length = prefix.length + before.length + after.length + (zcomma ?
            0 : negative.length),
  ...
  var d3_format_types = d3.map({
    ...
    r: function(x, p) {
      return (x = d3.round(x, d3_format_precision(x, p))).toFixed(Math.max(
        0, Math.min(20, d3_format_precision(x * (1 + 1e-15), p))));
    }
  });
  ...
  function d3_locale_timeFormat(locale) {
    ...
    function d3_time_format(template) {
      ...
      format.parse = function(string) {
        ...
        if ("j" in d) date.setFullYear(d.y, 0, d.j);
          ...
          date.setFullYear(d.y, 0, "W" in d ? (d.w + 6) % 7 + d.W * 7 - (date
            .getDay() + 5) % 7 : d.w + d.U * 7 - (date.getDay() + 6) % 7);
    ...
    function d3_time_parseMonth(date, string, i) {
      ...
      return n ? (date.m = d3_time_monthLookup.get(n[0].toLowerCase()), i + n[0]
        .length) : -1;
    }
    ...
    return function(feature) {
      ...
      if (n) {
        ...
        for (var n = merged.length - 1, i = 0, a = merged[n], b; i <= n; a =
          b, ++i) {
          b = merged[i];
          if ((dλ = angle(a[1], b[0])) > best) best = dλ, λ0 = b[0], λ1 = a[
            1];
        }
      }
      ranges = range = null;
  ...
  d3.geo.centroid = function(object) {
    ...
    if (m < ε2) {
      ...
      if (d3_geo_centroidW1 < ε) x = d3_geo_centroidX0, y = d3_geo_centroidY0,
        z = d3_geo_centroidZ0;
      m = x * x + y * y + z * z;
  ...
  function d3_geo_centroidLineStart() {
    ...
    function nextPoint(λ, φ) {
      ...
      var cosφ = Math.cos(φ *= d3_radians),
        ...
        y = cosφ * Math.sin(λ),
        z = Math.sin(φ),
        w = Math.atan2(Math.sqrt((w = y0 * z - z0 * y) * w + (w = z0 * x - x0 *
          z) * w + (w = x0 * y - y0 * x) * w), x0 * x + y0 * y + z0 * z);
      d3_geo_centroidW1 += w;
  ...
  function d3_geo_clipPolygon(segments, compare, clipStartInside, interpolate,
    ...
    segments.forEach(function(segment) {
      ...
      if (d3_geo_sphericalEqual(p0, p1)) {
        listener.lineStart();
        for (var i = 0; i < n; ++i) listener.point((p0 = segment[i])[0], p0[
          1]);
        listener.lineEnd();
        return;
      }
    ...
    return function(rotate, listener) {
      ...
      var clip = {
        ...
        polygonEnd: function() {
          ...
          var clipStartInside = d3_geo_pointInPolygon(rotatedClipStart,
            polygon);
          if (segments.length) {
            if (!polygonStarted) listener.polygonStart(), polygonStarted =
              true;
            d3_geo_clipPolygon(segments, d3_geo_clipSort, clipStartInside,
  ...
  function d3_geo_clipSort(a, b) {
    return ((a = a.x)[0] < 0 ? a[1] - halfπ - ε : halfπ - a[1]) - ((b = b.x)[0] <
      0 ? b[1] - halfπ - ε : halfπ - b[1]);
  }
  var d3_geo_clipAntimeridian = d3_geo_clip(d3_true,
    d3_geo_clipAntimeridianLine, d3_geo_clipAntimeridianInterpolate, [-π, -π /
      2
    ]);
  ...
  function d3_geo_clipAntimeridianIntersect(λ0, φ0, λ1, φ1) {
    ...
    return abs(sinλ0_λ1) > ε ? Math.atan((Math.sin(φ0) * (cosφ1 = Math.cos(φ1)) *
        Math.sin(λ1) - Math.sin(φ1) * (cosφ0 = Math.cos(φ0)) * Math.sin(λ0)) /
      (cosφ0 * cosφ1 * sinλ0_λ1)) : (φ0 + φ1) / 2;
  }
  ...
  function d3_geo_pointInPolygon(point, polygon) {
    ...
    for (var i = 0, n = polygon.length; i < n; ++i) {
      ...
      while (true) {
        ...
        var λ = point[0],
          ...
          antimeridian = adλ > π,
          k = sinφ0 * sinφ;
        d3_geo_areaRingSum.add(Math.atan2(k * sdλ * Math.sin(adλ), cosφ0 * cosφ +
          k * Math.cos(adλ)));
    ...
    return (polarAngle < -ε || polarAngle < ε && d3_geo_areaRingSum < -ε) ^
      winding & 1;
  }
  ...
  function d3_geo_clipCircle(radius) {
      ...
      return {
        ...
        point: function(λ, φ) {
          var point1 = [λ, φ],
            ...
            c = smallRadius ? v ? 0 : code(λ, φ) : v ? code(λ + (λ < 0 ? π :
              -π), φ) : 0;
    ...
    function intersect(a, b, two) {
      ...
      if (meridian ? polar ? φ0 + φ1 > 0 ^ q[1] < (abs(q[0] - λ0) < ε ? φ0 : φ1) :
        φ0 <= q[1] && q[1] <= φ1 : δλ > π ^ (λ0 <= q[0] && q[0] <= λ1)) {
  ...
  d3.geo.clipExtent = function() {
    var x0, y0, x1, y1, stream, clip, clipExtent = {
      ...
      extent: function(_) {
        ...
        clip = d3_geo_clipExtent(x0 = +_[0][0], y0 = +_[0][1], x1 = +_[1]
          [0], y1 = +_[1][1]);
  ...
  function d3_geo_clipExtent(x0, y0, x1, y1) {
    return function(listener) {
      ...
      function insidePolygon(p) {
        var wn = 0,
          ...
          y = p[1];
        for (var i = 0; i < n; ++i) {
          for (var j = 1, v = polygon[i], m = v.length, a = v[0], b; j < m; ++
            j) {
            b = v[j];
            if (a[1] <= y) {
    ...
    function corner(p, direction) {
      return abs(p[0] - x0) < ε ? direction > 0 ? 0 : 3 : abs(p[0] - x1) < ε ?
        direction > 0 ? 2 : 1 : abs(p[1] - y0) < ε ? direction > 0 ? 1 : 0 :
        direction > 0 ? 3 : 2;
    }
    ...
    function comparePoints(a, b) {
      var ca = corner(a, 1),
        cb = corner(b, 1);
      return ca !== cb ? ca - cb : ca === 0 ? b[1] - a[1] : ca === 1 ? a[0] - b[
        0] : ca === 2 ? a[1] - b[1] : b[0] - a[0];
    }
  }
  ...
  function d3_geo_conicEqualArea(φ0, φ1) {
    ...
    forward.invert = function(x, y) {
      var ρ0_y = ρ0 - y;
      return [Math.atan2(x, ρ0_y) / n, d3_asin((C - (x * x + ρ0_y * ρ0_y) * n *
        n) / (2 * n))];
    };
    return forward;
  }
  ...
  d3.geo.albersUsa = function() {
    ...
    var alaska = d3.geo.conicEqualArea().rotate([154, 0]).center([-2, 58.5]).parallels(
      [55, 65]);
    var hawaii = d3.geo.conicEqualArea().rotate([157, 0]).center([-3, 19.9]).parallels(
      [8, 18]);
    ...
    albersUsa.invert = function(coordinates) {
      ...
      return (y >= .12 && y < .234 && x >= -.425 && x < -.214 ? alaska : y >=
        .166 && y < .234 && x >= -.214 && x < -.115 ? hawaii : lower48).invert(
        coordinates);
    };
  ...
  var d3_geo_pathAreaSum, d3_geo_pathAreaPolygon, d3_geo_pathArea = {
    ...
    polygonEnd: function() {
      d3_geo_pathArea.lineStart = d3_geo_pathArea.lineEnd = d3_geo_pathArea
        .point = d3_noop;
    ...
    function resampleLineTo(x0, y0, λ0, a0, b0, c0, x1, y1, λ1, a1, b1, c1,
      ...
      if (d2 > 4 * δ2 && depth--) {
        var a = a0 + a1,
          ...
          φ2 = Math.asin(c /= m),
          λ2 = abs(abs(c) - 1) < ε || abs(λ0 - λ1) < ε ? (λ0 + λ1) / 2 : Math.atan2(
            b, a),
          p = project(λ2, φ2),
          x2 = p[0],
          y2 = p[1],
        ...
        if (dz * dz / d2 > δ2 || abs((dx * dx2 + dy * dy2) / d2 - .5) > .3 ||
          ...
          resampleLineTo(x0, y0, λ0, a0, b0, c0, x2, y2, λ2, a /= m, b /= m, c,
            depth, stream);
          stream.point(x2, y2);
  ...
  d3.geo.path = function() {
    ...
    function path(object) {
      if (object) {
        ...
        if (!cacheStream || !cacheStream.valid) cacheStream = projectStream(
          contextStream);
    ...
    path.context = function(_) {
      ...
      contextStream = (context = _) == null ? new d3_geo_pathBuffer() : new d3_geo_pathContext(
        _);
      if (typeof pointRadius !== "function") contextStream.pointRadius(
        pointRadius);
      return reset();
  ...
  d3.geo.rotation = function(rotate) {
    ...
    forward.invert = function(coordinates) {
      coordinates = rotate.invert(coordinates[0] * d3_radians, coordinates[
        1] * d3_radians);
      return coordinates[0] *= d3_degrees, coordinates[1] *= d3_degrees,
        coordinates;
    };
  ...
  function d3_geo_rotationφγ(δφ, δγ) {
    ...
    function rotation(λ, φ) {
      var cosφ = Math.cos(φ),
        ...
        z = Math.sin(φ),
        k = z * cosδφ + x * sinδφ;
      return [Math.atan2(y * cosδγ - k * sinδγ, x * cosδφ - z * sinδφ), d3_asin(
        k * cosδγ + y * sinδγ)];
    }
  ...
  d3.geo.circle = function() {
    ...
    circle.precision = function(_) {
      ...
      interpolate = d3_geo_circleInterpolate(angle * d3_radians, (precision = +
        _) * d3_radians);
      return circle;
    };
  ...
  d3.geo.distance = function(a, b) {
    var Δλ = (b[0] - a[0]) * d3_radians,
      ...
      cosφ1 = Math.cos(φ1),
      t;
    return Math.atan2(Math.sqrt((t = cosφ1 * sinΔλ) * t + (t = cosφ0 * sinφ1 -
      sinφ0 * cosφ1 * cosΔλ) * t), sinφ0 * sinφ1 + cosφ0 * cosφ1 * cosΔλ);
  };
  ...
  d3.geo.interpolate = function(source, target) {
    return d3_geo_interpolate(source[0] * d3_radians, source[1] * d3_radians,
  ...
  function d3_geo_interpolate(x0, y0, x1, y1) {
    ...
    var interpolate = d ? function(t) {
      var B = Math.sin(t *= d) * k,
        ...
        y = A * ky0 + B * ky1,
        z = A * sy0 + B * sy1;
      return [Math.atan2(y, x) * d3_degrees, Math.atan2(z, Math.sqrt(x * x +
        y * y)) * d3_degrees];
    } : function() {
  ...
  function d3_geo_lengthLineStart() {
    ...
    d3_geo_length.point = function(λ, φ) {
      λ0 = λ * d3_radians, sinφ0 = Math.sin(φ *= d3_radians), cosφ0 = Math.cos(
        φ);
    ...
    function nextPoint(λ, φ) {
      ...
      d3_geo_lengthSum += Math.atan2(Math.sqrt((t = cosφ * Math.sin(t)) * t + (
          t = cosφ0 * sinφ - sinφ0 * cosφ * cosΔλ) * t), sinφ0 * sinφ + cosφ0 *
        cosφ * cosΔλ);
  ...
  function d3_geo_conicConformal(φ0, φ1) {
    ...
    forward.invert = function(x, y) {
      ...
      return [Math.atan2(x, ρ0_y) / n, 2 * Math.atan(Math.pow(F / ρ, 1 / n)) -
        halfπ
      ];
    };
    return forward;
  ...
  function d3_geo_conicEquidistant(φ0, φ1) {
    ...
    forward.invert = function(x, y) {
      var ρ0_y = G - y;
      return [Math.atan2(x, ρ0_y) / n, G - d3_sgn(n) * Math.sqrt(x * x + ρ0_y *
        ρ0_y)];
    };
    return forward;
  }
  ...
  d3.geom.hull = function(vertices) {
    ...
    function hull(data) {
      ...
      var fx = d3_functor(x),
        ...
        flippedPoints = [];
      for (i = 0; i < n; i++) {
        points.push([+fx.call(this, data[i], i), +fy.call(this, data[i], i),
          i
        ]);
      }
      ...
      var skipLeft = lower[0] === upper[0],
        ...
        polygon = [];
      for (i = upper.length - 1; i >= 0; --i) polygon.push(data[points[upper[
        i]][2]]);
  ...
  function d3_geom_voronoiRemoveBeach(beach) {
    ...
    var lArc = previous;
    while (lArc.circle && abs(x - lArc.circle.x) < ε && abs(y - lArc.circle.cy) <
      ε) {
      previous = lArc.P;
    ...
    var rArc = next;
    while (rArc.circle && abs(x - rArc.circle.x) < ε && abs(y - rArc.circle.cy) <
      ε) {
      next = rArc.N;
  ...
  function d3_geom_voronoiLeftBreakPoint(arc, directrix) {
    ...
    var hl = lfocx - rfocx,
      aby2 = 1 / pby2 - 1 / plby2,
      b = hl / plby2;
    if (aby2) return (-b + Math.sqrt(b * b - 2 * aby2 * (hl * hl / (-2 * plby2) -
      lfocy + plby2 / 2 + rfocy - pby2 / 2))) / aby2 + rfocx;
  ...
  function d3_geom_voronoiCloseCells(extent) {
    ...
    while (iCell--) {
      ...
      while (iHalfEdge < nHalfEdges) {
        ...
        start = halfEdges[++iHalfEdge % nHalfEdges].start(), x2 = start.x, y2 =
          start.y;
  ...
  function d3_geom_voronoiClipEdges(extent) {
    var edges = d3_geom_voronoiEdges,
      clip = d3_geom_clipLine(extent[0][0], extent[0][1], extent[1][0], extent[
        1][1]),
      i = edges.length,
      e;
    while (i--) {
      e = edges[i];
      if (!d3_geom_voronoiConnectEdge(e, extent) || !clip(e) || abs(e.a.x - e.b
          .x) < ε && abs(e.a.y - e.b.y) < ε) {
        e.a = e.b = null;
        edges.splice(i, 1);
      }
    }
  ...
  function d3_geom_voronoiCreateEdge(lSite, rSite, va, vb) {
    ...
    d3_geom_voronoiCells[lSite.i].edges.push(new d3_geom_voronoiHalfEdge(edge,
      lSite, rSite));
    d3_geom_voronoiCells[rSite.i].edges.push(new d3_geom_voronoiHalfEdge(edge,
      rSite, lSite));
    return edge;
  }
  ...
  function d3_geom_voronoiHalfEdge(edge, lSite, rSite) {
    ...
    this.angle = rSite ? Math.atan2(rSite.y - lSite.y, rSite.x - lSite.x) :
      edge.l === lSite ? Math.atan2(vb.x - va.x, va.y - vb.y) : Math.atan2(va.x -
        vb.x, vb.y - va.y);
  }
  ...
  function d3_geom_voronoi(sites, bbox) {
    ...
    while (true) {
      ...
      if (site && (!circle || site.y < circle.y || site.y === circle.y && site.x <
          circle.x)) {
    ...
    voronoi.triangles = function(data) {
      ...
      d3_geom_voronoi(sites(data)).cells.forEach(function(cell, i) {
        ...
        while (++j < m) {
          ...
          e1 = edges[j].edge;
          s1 = e1.l === site ? e1.r : e1.l;
          if (i < s0.i && i < s1.i && d3_geom_voronoiTriangleArea(site,
              s0, s1) < 0) {
  ...
  d3.geom.quadtree = function(points, x1, y1, x2, y2) {
    ...
    function quadtree(data) {
      ...
      root.find = function(point) {
        return d3_geom_quadtreeFind(root, point[0], point[1], x1_, y1_, x2_,
          y2_);
      };
      i = -1;
      if (x1 == null) {
  ...
  d3.interpolators = [function(a, b) {
    var t = typeof b;
    return (t === "string" ? d3_rgb_names.has(b.toLowerCase()) ||
      /^(#|rgb\(|hsl\()/i.test(b) ? d3_interpolateRgb :
      d3_interpolateString : b instanceof d3_color ? d3_interpolateRgb :
      Array.isArray(b) ? d3_interpolateArray : t === "object" && isNaN(b) ?
    ...
    force.tick = function() {
      ...
      for (i = 0; i < m; ++i) {
        ...
        if (l = x * x + y * y) {
          ...
          x *= l;
          y *= l;
          t.x -= x * (k = s.weight + t.weight ? s.weight / (s.weight + t.weight) :
            .5);
          t.y -= y * k;
          s.x += x * (k = 1 - k);
          s.y += y * k;
  ...
  d3.layout.hierarchy = function() {
    ...
    hierarchy.revalue = function(root) {
      if (value) {
        ...
        d3_layout_hierarchyVisitAfter(root, function(node) {
          var parent;
          if (!node.children) node.value = +value.call(hierarchy, node,
            node.depth) || 0;
  ...
  d3.layout.pie = function() {
    ...
    function pie(data) {
      var n = data.length,
        ...
        da = (typeof endAngle === "function" ? endAngle.apply(this, arguments) :
          endAngle) - a,
  ...
  d3.layout.stack = function() {
    ...
    function stack(data, index) {
      ...
      for (j = 0; j < m; ++j) {
        ...
        for (i = 1; i < n; ++i) {
          out.call(stack, series[i][j], o += points[i - 1][j][1], points[i][j]
            [1]);
        }
      }
      return data;
    ...
    wiggle: function(data) {
      ...
      for (j = 1; j < m; ++j) {
        ...
        for (i = 0, s2 = 0, dx = x[j][0] - x[j - 1][0]; i < n; ++i) {
          for (k = 0, s3 = (data[i][j][1] - data[i][j - 1][1]) / (2 * dx); k <
            i; ++k) {
  ...
  d3.layout.pack = function() {
    ...
    function pack(d, i) {
      ...
      if (padding) {
        var dr = padding * (radius ? 1 : Math.max(2 * root.r / w, 2 * root.r /
          h)) / 2;
        d3_layout_hierarchyVisitAfter(root, function(d) {
          d.r += dr;
        });
      ...
      d3_layout_packTransform(root, w / 2, h / 2, radius ? 1 : 1 / Math.max(2 *
        root.r / w, 2 * root.r / h));
      return nodes;
    }
    ...
    function apportion(v, w, ancestor) {
      if (w) {
        ...
        while (vim = d3_layout_treeRight(vim), vip = d3_layout_treeLeft(vip),
          ...
          if (shift > 0) {
            d3_layout_treeMove(d3_layout_treeAncestor(vim, v, ancestor), v,
              shift);
            sip += shift;
            sop += shift;
          }
  ...
  function d3_layout_clusterRight(node) {
    var children = node.children,
      n;
    return children && (n = children.length) ? d3_layout_clusterRight(children[
      n - 1]) : node;
  }
    ...
    function squarify(node) {
      ...
      if (children && children.length) {
        var rect = pad(node),
          ...
          mode === "slice-dice" ? node.depth & 1 ? rect.dy : rect.dx : Math.min(
            rect.dx, rect.dy),
          n;
    ...
    treemap.padding = function(x) {
      ...
      function padFunction(node) {
        ...
        return p == null ? d3_layout_treemapPadNull(node) :
          d3_layout_treemapPad(node, typeof p === "number" ? [p, p, p, p] :
            p);
      }
      ...
      pad = (padding = x) == null ? d3_layout_treemapPadNull : (type =
        typeof x) === "function" ? padFunction : type === "number" ? (x = [
          x, x, x, x
        ],
  ...
  function d3_scale_linearTickFormat(domain, m, format) {
    ...
    if (format) {
      ...
      if (!match[7]) match[7] = "." + d3_scale_linearFormatPrecision(match[8],
        range);
      format = match.join("");
    } else {
  ...
  function d3_scale_ordinal(domain, ranger) {
    ...
    scale.rangeRoundPoints = function(x, padding) {
      ...
      var start = x[0],
        stop = x[1],
        step = domain.length < 2 ? (start = stop = Math.round((start + stop) /
            2),
  ...
  var d3_category10 = [2062260, 16744206, 2924588, 14034728, 9725885, 9197131,
    14907330, 8355711, 12369186, 1556175
  ].map(d3_rgbString);
  var d3_category20 = [2062260, 11454440, 16744206, 16759672, 2924588, 10018698,
    14034728, 16750742, 9725885, 12955861, 9197131, 12885140, 14907330,
    16234194, 8355711, 13092807, 12369186, 14408589, 1556175, 10410725
  ].map(d3_rgbString);
  var d3_category20b = [3750777, 5395619, 7040719, 10264286, 6519097, 9216594,
    11915115, 13556636, 9202993, 12426809, 15186514, 15190932, 8666169,
  ...
  d3.svg.arc = function() {
    ...
    function arc() {
      ...
      if (da >= τε) return circleSegment(r1, cw) + (r0 ? circleSegment(r0, 1 -
        cw) : "") + "Z";
      ...
      if (da > ε && (rc = Math.min(Math.abs(r1 - r0) / 2, +cornerRadius.apply(
          this, arguments))) > .001) {
        cr = r0 < r1 ^ cw ? 0 : 1;
        var rc1 = rc,
          rc0 = rc;
        ...
        if (x1 != null) {
          var t30 = d3_svg_arcCornerTangents(x3 == null ? [x2, y2] : [x3, y3], [
              x0, y0
            ], r1, rc1, cw),
            t12 = d3_svg_arcCornerTangents([x1, y1], [x2, y2], r1, rc1, cw);
          if (rc === rc1) {
            path.push("M", t30[0], "A", rc1, ",", rc1, " 0 0,", cr, " ", t30[
                1], "A", r1, ",", r1, " 0 ", 1 - cw ^ d3_svg_arcSweep(t30[1]
                [0], t30[1][1], t12[1][0], t12[1][1]), ",", cw, " ", t12[1],
              "A", rc1, ",", rc1, " 0 0,", cr, " ", t12[0]);
          } else {
            path.push("M", t30[0], "A", rc1, ",", rc1, " 0 1,", cr, " ", t12[
              0]);
          }
        } else {
          path.push("M", x0, ",", y0);
        ...
        if (x3 != null) {
          ...
          if (rc === rc0) {
            path.push("L", t21[0], "A", rc0, ",", rc0, " 0 0,", cr, " ", t21[
                1], "A", r0, ",", r0, " 0 ", cw ^ d3_svg_arcSweep(t21[1][0],
                t21[1][1], t03[1][0], t03[1][1]), ",", 1 - cw, " ", t03[1],
              "A", rc0, ",", rc0, " 0 0,", cr, " ", t03[0]);
          } else {
            path.push("L", t21[0], "A", rc0, ",", rc0, " 0 0,", cr, " ", t03[
              0]);
          }
        } else {
          path.push("L", x2, ",", y2);
        ...
        if (x1 != null) path.push("A", r1, ",", r1, " 0 ", l1, ",", cw, " ",
          x1, ",", y1);
        path.push("L", x2, ",", y2);
        if (x3 != null) path.push("A", r0, ",", r0, " 0 ", l0, ",", 1 - cw,
          " ", x3, ",", y3);
      }
      path.push("Z");
      return path.join("");
    ...
    function circleSegment(r1, cw) {
      return "M0," + r1 + "A" + r1 + "," + r1 + " 0 1," + cw + " 0," + -r1 +
        "A" + r1 + "," + r1 + " 0 1," + cw + " 0," + r1;
    }
  ...
  function d3_svg_lineCardinalClosed(points, tension) {
    return points.length < 3 ? d3_svg_lineLinearClosed(points) : points[0] +
  ...
  function d3_svg_lineHermite(points, tangents) {
    ...
    if (tangents.length > 1) {
      ...
      p = points[pi];
      pi++;
      path += "C" + (p0[0] + t0[0]) + "," + (p0[1] + t0[1]) + "," + (p[0] - t[0]) +
        "," + (p[1] - t[1]) + "," + p[0] + "," + p[1];
  ...
  function d3_svg_lineBasisClosed(points) {
    ...
    path = [d3_svg_lineDot4(d3_svg_lineBasisBezier3, px), ",", d3_svg_lineDot4(
      d3_svg_lineBasisBezier3, py)];
    --i;
    while (++i < m) {
      pi = points[i % n];
      px.shift();
  ...
  function d3_svg_lineMonotoneTangents(points) {
    ...
    i = -1;
    while (++i <= j) {
      s = (points[Math.min(j, i + 1)][0] - points[Math.max(0, i - 1)][0]) / (6 *
        (1 + m[i] * m[i]));
  ...
  d3.svg.chord = function() {
    ...
    function chord(d, i) {
      ...
      return "M" + s.p0 + arc(s.r, s.p1, s.a1 - s.a0) + (equals(s, t) ? curve(
        s.r, s.p1, s.r, s.p0) : curve(s.r, s.p1, t.r, t.p0) + arc(t.r, t.p1,
        t.a1 - t.a0) + curve(t.r, t.p1, s.r, s.p0)) + "Z";
    }
  ...
  d3.svg.diagonal.radial = function() {
    ...
    diagonal.projection = function(x) {
      return arguments.length ? projection_(d3_svg_diagonalRadialProjection(
  ...
  function d3_svg_symbolCircle(size) {
    ...
    return "M0," + r + "A" + r + "," + r + " 0 1,1 0," + -r + "A" + r + "," + r +
      " 0 1,1 0," + r + "Z";
  }
  var d3_svg_symbols = d3.map({
    ...
    cross: function(size) {
      ...
      return "M" + -3 * r + "," + -r + "H" + -r + "V" + -3 * r + "H" + r +
        "V" + -r + "H" + 3 * r + "V" + r + "H" + r + "V" + 3 * r + "H" +
        -r + "V" + r + "H" + -3 * r + "Z";
    },
  ...
  d3.transition = function(selection, name) {
    return selection && selection.transition ? d3_transitionInheritId ?
      selection.transition(name) : selection : d3.selection().transition(
        selection);
  };
      ...
      for (var group = this[j], i = -1, n = group.length; ++i < n;) {
        if (node = group[i]) {
          ...
          for (var k = -1, o = subnodes.length; ++k < o;) {
            if (subnode = subnodes[k]) d3_transitionNode(subnode, k, ns, id,
              transition);
            subgroup.push(subnode);
          }
        }
  ...
  function d3_transition_tween(groups, name, value, tween) {
    ...
    return d3_selection_each(groups, typeof value === "function" ? function(
      node, i, j) {
      node[ns][id].tween.set(name, tween(value.call(node, node.__data__, i,
        j)));
  ...
  d3_transitionPrototype.style = function(name, value, priority) {
    ...
    function styleString(b) {
      return b == null ? styleNull : (b += "", function() {
        var a = d3_window(this).getComputedStyle(this, null).getPropertyValue(
            name),
          i;
  ...
  d3_transitionPrototype.styleTween = function(name, tween, priority) {
    ...
    function styleTween(d, i) {
      var f = tween.call(this, d, i, d3_window(this).getComputedStyle(this,
  ...
  function d3_transitionNode(node, i, ns, id, inherit) {
    ...
    function start(elapsed) {
      ...
      if (active) {
        ...
        active.event && active.event.interrupt.call(node, node.__data__, active
          .index);
      }
    ...
    function axis(g) {
      g.each(function() {
        ...
        var ticks = tickValues == null ? scale1.ticks ? scale1.ticks.apply(
          ...
          tickFormat = tickFormat_ == null ? scale1.tickFormat ? scale1.tickFormat
          .apply(scale1, tickArguments_) : d3_identity : tickFormat_,
          tick = g.selectAll(".tick").data(ticks, scale1),
          tickEnter = tick.enter().insert("g", ".domain").attr("class",
            "tick").style("opacity", ε),
        ...
        if (orient === "bottom" || orient === "top") {
          tickTransform = d3_svg_axisX, x1 = "x", y1 = "y", x2 = "x2", y2 =
            "y2";
          text.attr("dy", sign < 0 ? "0em" : ".71em").style("text-anchor",
            "middle");
          pathUpdate.attr("d", "M" + range[0] + "," + sign *
            outerTickSize + "V0H" + range[1] + "V" + sign *
            outerTickSize);
        } else {
          tickTransform = d3_svg_axisY, x1 = "y", y1 = "x", x2 = "y2", y2 =
            "x2";
          text.attr("dy", ".32em").style("text-anchor", sign < 0 ? "end" :
            "start");
          pathUpdate.attr("d", "M" + sign * outerTickSize + "," + range[0] +
            "H0V" + range[1] + "H" + sign * outerTickSize);
        }
  ...
  d3.svg.brush = function() {
    ...
    function brush(g) {
      g.each(function() {
        ...
        background.enter().append("rect").attr("class", "background").style(
          "visibility", "hidden").style("cursor", "crosshair");
    ...
    function brushstart() {
      var target = this,
        ...
        offset;
      var w = d3.select(d3_window(target)).on("keydown.brush", keydown).on(
        "keyup.brush", keyup);
      ...
      g.style("pointer-events", "none").selectAll(".resize").style("display",
        null);
      ...
      function brushend() {
        brushmove();
        g.style("pointer-events", "all").selectAll(".resize").style("display",
    ...
    brush.clamp = function(z) {
      if (!arguments.length) return x && y ? [xClamp, yClamp] : x ? xClamp :
        y ? yClamp : null;
  ...
  function d3_time_scale(linear, methods, format) {
    ...
    function tickMethod(extent, count) {
      ...
      ] : !i ? [d3_time_scaleMilliseconds, d3_scale_linearTickRange(extent,
        count)[2]] : methods[target / d3_time_scaleSteps[i - 1] <
